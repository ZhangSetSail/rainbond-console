...
class DockerRunCreateView(RegionTenantHeaderView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if is_demo:
            groups = ServiceGroup.objects.filter(tenant_id=self.tenant.tenant_id, region_name=self.region_name, group_name="镜像构建示例")
            k8s_app_name = "image-demo"
            if groups:
                group_id = groups[0].ID
            else:
                k8s_apps = ServiceGroup.objects.filter(tenant_id=self.tenant.tenant_id, region_name=self.region_name, k8s_app="image-demo")
                if k8s_apps:
                    k8s_app_name += make_uuid()[:6]
                data = group_service.create_app(self.tenant, self.region_name, "镜像构建示例", None, self.user.get_username(),
