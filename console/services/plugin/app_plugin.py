...
class AppPluginService(object):
    ...
    def install_new_plugin(self, region, tenant, service, plugin_id, plugin_version=None, user=None):
        ...
        plugin_rel = self.create_service_plugin_relation(tenant.tenant_id, service.service_id, plugin_id,
                                                         plugin_version)
    ...
    def __create_service_monitor(tenant, service, plugin_name, user=None):
        ...
        try:
            service_monitor_repo.create_component_service_monitor(tenant, service, "mysqldexporter-" + make_uuid()[0:4],
                                                                  path,
    ...
    def build_plugin_data_4marketsvc(self, tenant, service, plugin):
        ...
        if not p:
            logger.warning(
                "open plugin failure , plugin {} not found in tenant {}".format(plugin_key, tenant.tenant_id))
            return
        plugin_id = p[0].plugin_id
        plugin_version = plugin_version_service.get_newest_plugin_version(tenant.tenant_id, plugin_id)
        if not plugin_version:
            logger.warning(
                "open plugin failure , plugin {} not build in tenant {}".format(plugin_key, tenant.tenant_id))
            return
    ...
    def add_default_plugin(self, user, tenant, region, plugin_type="perf_analyze_plugin"):
        ...
        try:
            ...
            plugin_build_version = plugin_version_service.create_build_version(
                region, plugin_base_info.plugin_id, tenant.tenant_id, user.user_id, "", "unbuild", 256,
                image_tag=image_tag)
